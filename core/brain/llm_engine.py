...
def _cuda_available() -> bool:
    """Detect CUDA properly instead of using sys.platform string."""
    try:
        import ctypes
...
def _resolve_model_path(model_path: str) -> Optional[str]:
    """Resolve model path once; returns absolute path or None."""
    candidates = [
        model_path,
...
class GPUMemoryGuard:
    ...
    def should_throttle(self, priority: int) -> bool:
        if not self.is_under_pressure():
            return False
        return priority >= AgentPriority.LOW.value
...
class VectorIndex:
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 3) -> List[str]:
        try:
            ...
            logger.debug(f"Vector search error: {exc}")
            return []
    ...
    def search(self, query: str, top_k: int = 3) -> List[str]:
        try:
            ...
            try:
                ...
                logger.debug(f"Embedding failed: {exc}")
                return []
            ...
            logger.debug(f"Retrieval search error: {exc}")
            return []
...
class LLMEngine:
    ...
    def _start_workers(self) -> None:
        ...
        logger.info(f"LLMEngine: started {_NUM_WORKERS} queue workers")

    async def _queue_worker(self, worker_id: int) -> None:
        logger.debug(f"LLM queue worker {worker_id} started")
        ...
        while self._worker_running and not self._shutdown:
            try:
                ...
                logger.error(f"Queue worker {worker_id} error: {exc}")
                if current_future and not current_future.done():
                    current_future.set_result({
        ...
        logger.debug(f"LLM queue worker {worker_id} stopped")
    ...
    async def _build_prompt(
        ...
        try:
            ...
            logger.debug(f"Prompt build error: {exc}")
        ...
        async with self._semaphore:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Prompt build failed: {exc}")
                    full_prompt = prompt
                ...
                for attempt in range(retries + 1):
                    try:
                        ...
                        logger.warning(f"Generate attempt {attempt + 1} failed: {exc}")
                        if attempt < retries:
        ...
        try:
            ...
            for attempt in range(retries + 1):
                try:
                    ...
                    logger.warning(f"safe_generate attempt {attempt + 1}: {exc}")
                if attempt < retries:
                    await asyncio.sleep(0.5 * (attempt + 1))

            return _safe_default
        except Exception as exc:
            logger.error(f"safe_generate failed: {exc}")
            return _safe_default
        ...
        try:
            if self.cache_enabled and hasattr(self, "cache"):
                try:
                    ...
                    logger.debug(f"Cache clear failed: {exc}")
            ...
            if self.semantic_enabled and hasattr(self, "semantic_memory"):
                try:
                    ...
                    logger.debug(f"Semantic clear failed: {exc}")
            ...
            elapsed = time.monotonic() - start
            logger.info(
                f"Context cleared | cache={cache_cleared} semantic={sem_cleared} time={elapsed:.4f}s"
            )
            return {
                "status": "cleared",
            ...
            logger.error(f"Context clear failed: {exc}")
    ...
    async def initialize(self, timeout: float = 30.0, *args, **kwargs) -> bool:
        ...
        elapsed = time.monotonic() - start
        logger.info(f"LLMEngine initialized in {elapsed:.2f}s ({_NUM_WORKERS} workers)")
        return True
...
class AgentPool:
    ...
    async def register(
        ...
        async with self._lock:
            ...
            active = sum(1 for s in self._slots.values() if s.state != AgentState.RETIRED)
            ...
            logger.info(
                f"AgentPool: registered '{agent_id}' (priority={priority.name}, rps={rate_limit:.1f}) "
                f"[{active + 1}/{MAX_AGENTS} active]"
            )
            return slot
    ...
    def pause(self, agent_id: str) -> bool:
        ...
        if slot and slot.state == AgentState.ACTIVE:
            ...
            logger.info(f"AgentPool: paused '{agent_id}'")
            return True
        return False
    ...
    def resume(self, agent_id: str) -> bool:
        ...
        if slot and slot.state == AgentState.PAUSED:
            ...
            logger.info(f"AgentPool: resumed '{agent_id}'")
            return True
        return False
    ...
    def retire(self, agent_id: str) -> bool:
        ...
        if slot:
            ...
            logger.info(f"AgentPool: retired '{agent_id}'")
            return True
        return False
    ...
    def get_pool_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requests": {
                ...
                "success_rate": round(total_completed / max(1, total_sent), 3),
            },
