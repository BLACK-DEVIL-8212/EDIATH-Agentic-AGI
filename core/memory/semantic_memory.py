...
class SemanticMemory(BaseMemory):
    ...
    def _save_to_db(self, knowledge: KnowledgeNode):
        ...
        try:
            ...
            if self.vector_db and hasattr(knowledge, 'embedding') and knowledge.embedding:
    ...
    def search_similar(self, content: str, threshold: float = 0.5, limit: int = 10) -> List[Dict[str, Any]]:
            ...
            for key, knowledge in self.knowledge_base.items():
                if isinstance(knowledge.content, str):
                    ...
                    if content_words and target_words:
                        ...
                        similarity = overlap / max(len(content_words), len(target_words))
    ...
    def delete(self, key: str, cascade: bool = False) -> bool:
        ...
        with self._knowledge_lock:
            ...
            try:
                if hasattr(mongo_client, 'collection'):
    ...
    def export_to_json(self, filepath: Optional[str] = None) -> str:
        ...
        logger.info(f"📤 Exported {len(export_data['knowledge'])} knowledge nodes to {filepath}")
        return filepath
