...
class EDIATHSystem:
    ...
    async def _process_user_input_async(self, user_input: str) -> None:
        # Ensure thinking is ALWAYS reset on exit
        try:
            ...
            if _import_agent_modules(self.shared_llm):
                try:
                    ...
                    itype = intent.type.value if hasattr(intent, "type") and hasattr(intent.type, "value") else "query"
                    routed = await asyncio.wait_for(
                        _action_router_instance.route(itype, {}), timeout=2
                    )
                    # ✅ FIX: Ensure routed is always a valid string, default to "llm"
                    if not routed or not isinstance(routed, str):
                        routed = "llm"
                    agent_key = _router_instance.route({"action": routed}) if routed else "llm"
                    # ✅ FIX: Map router output to agent, with llm as default
                    agent_map = {
                        "search": self.search_agent,
                        "codeexecution": self.code_agent,
                    ...
                    agent = agent_map.get(agent_key) or self.agent
                    self.logger.info("Routing → intent=%s, routed=%s, agent_key=%s", itype, routed, agent_key)
                except Exception as exc:
                    self.logger.warning("Routing failed → using default agent: %s", exc)

            vision_context = ""
            memory_context = ""

            try:
                if self.vision_engine_instance and hasattr(self.vision_engine_instance, "get_perception_result"):
                    vdata = await asyncio.wait_for(
                        self.vision_engine_instance.get_perception_result(), timeout=2
                    )
                    if vdata and (vdata.get("detections") or vdata.get("caption") or vdata.get("faces")):
                        vision_context = (
                            await asyncio.wait_for(
                ...
                self.logger.debug("Vision context error: %s", exc)
            ...
            try:
                ...
                self.logger.warning("Decision engine timeout - LLM may be busy")
            ...
            # CRITICAL: Always reset thinking flag
            self.thinking = False
...
def main_web() -> None:
    ...
    def init_in_thread():
        ...
        try:
            try:
                # ✅ FIX: Increased timeout for slow model loading on CPU
                init_success = loop.run_until_complete(
                    asyncio.wait_for(system.initialize(), timeout=180)
                )
            except asyncio.TimeoutError:
                # ✅ FIX: More permissive partial connect - allow if LLM is ready or any agent exists
                logger.warning("⚠ system.initialize() exceeded 180 s — attempting partial connect")
                if system.agent or system.llm_ready:
                    system.is_running = True
                    init_success = True
                    logger.info("Partial connect — system available, continuing")
                else:
                    init_success = False
